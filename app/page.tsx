...
export default function Home() {
  ...
  const [questions, setQuestions] = useState<Question[]>(() => shuffleArray(questionsBase));
  const [step, setStep] = useState(0);
  const [answers, setAnswers] = useState<(QuestionOption | null)[]>(
    Array(questionsBase.length).fill(null)
  );
  ...
  const openDiagnosis = () => {
    const shuffledQuestions = shuffleArray(questionsBase);
