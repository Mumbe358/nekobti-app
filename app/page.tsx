...
export default function Home() {
  ...
  const fetchTypeShares = async () => {
    try {
      const response = await fetch("/api/analytics/type-shares", {
        method: "GET",
        cache: "no-store",
      });

      const data = await readJsonSafely<TypeSharesResponse>(response);

      if (!response.ok || !data?.ok || !data.typeShares) {
        console.error("type share fetch failed:", data?.error ?? response.statusText);
        return;
      }
